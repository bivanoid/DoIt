	...
	return (
		<div className={s.con_title}>
			<div className={s.title}>
				...
				<h1>{clock.time}</h1>
				<p className={s.dates}>
					<span>[{clock.day}]</span> {clock.date}
				</p>
			</div>
			{!menggantiKota && (
				<div className={s.wheater}>
					{weather.isOffline ? (
						...
						<>
							...
							<p className={s.suhu}>
								<button
									disabled={rotateRefresh}
									className={`${s.switch_temperature} ${rotateRefresh ? s.rotated : ""}`}
									onClick={handleRefreshAnim}
								>
									<RefreshCw
										className={rotateRefresh ? s.spin : ""}
										size={15}
									/>
								</button>
								{switchSuhu ? weather.temp_f : weather.temp_c}°{" "}
								{switchSuhu ? "F" : "C"} {temperature}
							</p>
						</>
					)}
				</div>
